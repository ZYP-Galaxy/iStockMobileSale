...
public class itemAdapter extends BaseAdapter {
    ...
    private double currencySale_price;
    boolean isCalCurr;
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (formname == "Sale") {
            ...
            if (sale_entry.sd.get(position).getBalance() < 0) {
...
//        }
        ...
        if (formname == "SaleOrder")//added by YLT
        {
            ...
            tv.setText(String.valueOf(returnin_entry.sd.get(position).getSr()));
            tv1.setText(returnin_entry.sd.get(position).getDesc());
            Double unit_qty = returnin_entry.sd.get(position).getUnit_qty();
            String qtyAsString = String.format("%." + frmmain.qty_places + "f", unit_qty);
            tv2.setText(qtyAsString);
            tv3.setText(returnin_entry.sd.get(position).getUnit_short());
            double amt = returnin_entry.sd.get(position).getSale_price() * returnin_entry.sd.get(position).getUnit_qty();
            String numberAsString = String.format("%,." + frmmain.price_places + "f", amt);
            tv4.setText(String.valueOf(numberAsString));
        } else {
           ...
           // amt = currencySale_price* sale_entry.sd.get(position).getUnit_qty();
            String numberAsString = String.format("%,." + frmmain.price_places + "f", amt);
            tv4.setText(String.valueOf(numberAsString));
        }
        ...
        tv2.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (formname == "Sale") {
                    if (sale_entry.sd.get(position).getBalance() < 0) {
                        AlertDialog.Builder cb = new AlertDialog.Builder(context);
                        cb.setTitle("iStock");
                        cb.setCancelable(false);
                        double exceed = (-1) * sale_entry.sd.get(position).getBalance();
                        String message = "This Code is not Enough Stock!";
                ...
                bd.setPositiveButton("OK", new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialog, int which) {
                        ...
                        if (formname == "SaleOrder")//added by YLT
                        {
                            ...
                            returnin_entry.sd.remove(position);
                            itemPosition = -1;
                            for (int i = 0; i < returnin_entry.sd.size(); i++) {
                                returnin_entry.sd.get(i).setSr(i + 1);
                            }
                            returnin_entry.getData();
                            returnin_entry.getSummary();
                            if (returnin_entry.sd.size() == 0) {
                                String tax = "Tax" + (returnin_entry.getTax() > 0 ? "( " + returnin_entry.getTax() + "% )" : "");
                                returnin_entry.txttax.setText(tax);
                                returnin_entry.sh.get(0).setTax_per(returnin_entry.getTax());
                                returnin_entry.sh.get(0).setTax_amount(0.0);
                                returnin_entry.sh.get(0).setDiscount(0.0);
                                returnin_entry.sh.get(0).setDiscount_per(0);
                                returnin_entry.sh.get(0).setPaid_amount(0);
                                returnin_entry.txtvoudis.setText("0");
        ...
        tv4.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (formname == "SaleOrder")//added by YLT
                {
                    ...
                    save.setOnClickListener(new View.OnClickListener() {
                        ...
                        public void onClick(View v) {
                            ...
                            saleorder_entry.sd.get(position).setOrgSalePrice(Double.parseDouble(ClearFormat(txtChangePrice.getText().toString())));//added by KLM for currency 27062022
                            saleorder_entry.sd.get(position).setChangeSalePrice(true);//added by KLM for currency 27062022
                    ...
                    String title = returnin_entry.sd.get(position).getDesc();
                    txtheader.setText(title);
                    TextView txtChangePrice = view.findViewById(R.id.txtChangePrice);
                    RelativeLayout rlAmount = view.findViewById(R.id.rlAmount);
                    rlAmount.setVisibility(View.INVISIBLE);
                    ...
                    ImageButton save = view.findViewById(R.id.imgSave);
                    //txtChangePrice.setText(String.valueOf(sale_entry.sd.get(position).getSale_price()));
                    txtChangePrice.setText(String.format("%,." + frmmain.price_places + "f", returnin_entry.sd.get(position).getSale_price()));
                    disamt = returnin_entry.sd.get(position).getSale_price() - returnin_entry.sd.get(position).getDis_price();
                    Double amt = Double.parseDouble(ClearFormat(txtChangePrice.getText().toString())) * returnin_entry.sd.get(position).getUnit_qty();
                    ...
                    txtChangePrice.setOnClickListener(new View.OnClickListener() {
                        ...
                        public void onClick(View v) {
                            //keynum=txtChangePrice.getText().toString();
                            int op = returnin_entry.sd.get(position).getOpen_price();
                            if (frmlogin.change_price == 1 || op == 1) {
                                keynum = String.format("%,." + frmmain.price_places + "f", Double.parseDouble(ClearFormat(txtChangePrice.getText().toString())));
                                showKeyPad(txtChangePrice, txtChangePrice, position);
                            }
                    ...
                    save.setOnClickListener(new View.OnClickListener() {
                        ...
                        public void onClick(View v) {
                            ...
                            returnin_entry.sd.get(position).setSale_price(Double.parseDouble(ClearFormat(txtChangePrice.getText().toString())));
                            returnin_entry.sd.get(position).setOrgSalePrice(Double.parseDouble(ClearFormat(txtChangePrice.getText().toString())));//added by KLM for currency 27062022
                            returnin_entry.sd.get(position).setChangeSalePrice(true);//added by KLM for currency 27062022
                            returnin_entry.sd.get(position).setDis_price(Double.parseDouble(ClearFormat(txtChangePrice.getText().toString())));

                            if (returnin_entry.sd.get(position).getDis_type() == 3
                                    || returnin_entry.sd.get(position).getDis_type() == 4
                                    || returnin_entry.sd.get(position).getDis_type() == 6
                                    || returnin_entry.sd.get(position).getDis_type() == 7) {
                                returnin_entry.sd.get(position).setDis_percent(0);
                                returnin_entry.sd.get(position).setDis_price(returnin_entry.sd.get(position).getSale_price());
                            } else if (returnin_entry.sd.get(position).getDis_type() == 1
                                    || returnin_entry.sd.get(position).getDis_type() == 2) {
                                double dispercent = returnin_entry.sd.get(position).getDis_type() == 1 ? 0.05 : 0.1;
                                double discount = returnin_entry.sd.get(position).getDis_type() == 1 ? 5 : 10;
                                returnin_entry.sd.get(position).setDis_percent(discount);
                                double dis_price = returnin_entry.sd.get(position).getSale_price() - (returnin_entry.sd.get(position).getSale_price() * (dispercent));
                                returnin_entry.sd.get(position).setDis_price(dis_price);
                            } else if (returnin_entry.sd.get(position).getDis_type() == 5) {
                                if (returnin_entry.sd.get(position).getDis_percent() > 0) {
                                    double dis_percent = returnin_entry.sd.get(position).getDis_percent();
                                    returnin_entry.sd.get(position).setDis_percent(dis_percent);
                                    double dis_price = returnin_entry.sd.get(position).getSale_price() - (returnin_entry.sd.get(position).getSale_price() * (dis_percent / 100));
                                    returnin_entry.sd.get(position).setDis_price(dis_price);
                                    ...
                                    double dis_percent = 0;
                                    returnin_entry.sd.get(position).setDis_percent(dis_percent);
                                    double dis_price = returnin_entry.sd.get(position).getSale_price() - disamt;
                                    returnin_entry.sd.get(position).setDis_price(dis_price);
                                }
                            }

                            returnin_entry.getSummary();
                            returnin_entry.entrygrid.setAdapter(itemAd);
                            returnin_entry.entrygrid.setSelection(position);
                            itemPosition = -1;
                            da.dismiss();
                ...
                //for Sale
                    ...
                    //disamt=currencySale_price-sale_entry.sd.get(position).getDis_price();
                    String numberAsString = String.format("%,." + frmmain.price_places + "f", amt);
                    txtamt.setText(numberAsString);
                    ...
                    save.setOnClickListener(new View.OnClickListener() {
                        ...
                        public void onClick(View v) {
                            ...
                            sd.get(position).setOrgSalePrice(Double.parseDouble(ClearFormat(txtChangePrice.getText().toString())));//added by KLM for currency 27062022
                            sd.get(position).setChangeSalePrice(true);//added by KLM for currency 27062022
            ...
            public void onClick(View v) {
                try {
                    ...
                    if (formname == "SaleOrder")//added by YLT
                    {
                        ...
                        sqlString = "select * from Usr_Code where unit>0 and code=" + returnin_entry.sd.get(position).getCode() + " order by unit_type";
                        cursor = DatabaseHelper.rawQuery(sqlString);
                    } else {
    ...
    public static void CalculateCurrencyPrice(long code, int position) {
        int sale_curr=1;
        ...
        if(cursor!=null && cursor.getCount()>0){
            if (cursor.moveToFirst()) {
                do {
                    sale_curr = cursor.getInt(cursor.getColumnIndex("sale_curr"));
        ...
        sqlString="select exg_rate,div_rate from Currency where currency="+sale_curr;
        ...
        //currencySale_price=sale_entry.sd.get(position).getSale_price();
        if(frmlogin.def_currency!=sale_curr){
            switch (formname){
                case "ReturnIn":
                    returnin_entry.sd.get(position).setSale_price(returnin_entry.sd.get(position).getOrgSalePrice()*currentCodeExg_rate/frmmain.exg_rate);
                    returnin_entry.itemAdapter.notifyDataSetChanged();
                    returnin_entry.getSummary();

                    break;
                case "SaleOrder":
                    saleorder_entry.sd.get(position).setSale_price(saleorder_entry.sd.get(position).getOrgSalePrice()*currentCodeExg_rate/frmmain.exg_rate);
                    saleorder_entry.itemAdapter.notifyDataSetChanged();
                    saleorder_entry.getSummary();

                    break;
                default:
                    sale_entry.sd.get(position).setSale_price(sale_entry.sd.get(position).getOrgSalePrice()*currentCodeExg_rate/frmmain.exg_rate);
                ...
                try {
                    ...
                    if (isqty) {
                        ...
                        if (formname == "SaleOrder") {
                            ...
                            check = check > 0 ? check : 1;
                            returnin_entry.sd.get(itemposition).setUnit_qty(check);
                            source.setText(String.valueOf(check));
                            GetSpecialPrice(itemposition);
                            returnin_entry.entrygrid.setAdapter(itemAd);
                        ...
                        if (formname == "SaleOrder") //added by YLT
                        {
                            ...
                            check = check > 0 ? check : 0;
                            returnin_entry.sd.get(itemposition).setSale_price(check);
                            Double amt = check * returnin_entry.sd.get(itemposition).getUnit_qty();
                            String numberAsString = String.format("%,." + frmmain.price_places + "f", amt);
                            txtamt.setText(numberAsString);
                            source.setText(String.format("%,." + frmmain.price_places + "f", check));
                            //sale_entry.getSummary();
    ...
    public void CalculateItemDiscount(int itemPosition, double dis) {
        ...
        if (formname == "SaleOrder")//added by YLT
        {
            ...
            if (returnin_entry.sd.get(itemPosition).getDis_type() == 3
                    || returnin_entry.sd.get(itemPosition).getDis_type() == 4
                    || returnin_entry.sd.get(itemPosition).getDis_type() == 6
                    || returnin_entry.sd.get(itemPosition).getDis_type() == 7) {
                returnin_entry.sd.get(itemPosition).setDis_percent(0);
                returnin_entry.sd.get(itemPosition).setDis_price(sd.get(itemPosition).getSale_price());
            } else if (returnin_entry.sd.get(itemPosition).getDis_type() == 1
                    || returnin_entry.sd.get(itemPosition).getDis_type() == 2) {
                double dispercent = returnin_entry.sd.get(itemPosition).getDis_type() == 1 ? 0.05 : 0.1;
                double discount = returnin_entry.sd.get(itemPosition).getDis_type() == 1 ? 5 : 10;
                returnin_entry.sd.get(itemPosition).setDis_percent(discount);
                double dis_price = returnin_entry.sd.get(itemPosition).getSale_price() - (sd.get(itemPosition).getSale_price() * (dispercent));
                returnin_entry.sd.get(itemPosition).setDis_price(dis_price);
            } else if (returnin_entry.sd.get(itemPosition).getDis_type() == 5) {
                if (returnin_entry.sd.get(itemPosition).getDis_percent() > 0) {
                    double dis_percent = returnin_entry.sd.get(itemPosition).getDis_percent();
                    returnin_entry.sd.get(itemPosition).setDis_percent(dis_percent);
                    double dis_price = returnin_entry.sd.get(itemPosition).getSale_price() - (returnin_entry.sd.get(itemPosition).getSale_price() * (dis_percent / 100));
                    sd.get(itemPosition).setDis_price(dis_price);
                    ...
                    double dis_percent = returnin_entry.sd.get(itemPosition).getDis_percent();
                    returnin_entry.sd.get(itemPosition).setDis_percent(dis_percent);
                    double dis_price = returnin_entry.sd.get(itemPosition).getSale_price() - dis;
                    returnin_entry.sd.get(itemPosition).setDis_price(dis_price);
                }
            }
        } else {
