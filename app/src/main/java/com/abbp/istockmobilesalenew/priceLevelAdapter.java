        ...
        holder.btn.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
             if(formname=="Sale Order")
             {
                 ...
                 //added by KLM for currency on priceLevel 28062022
                 if(frmmain.use_multicurrency){
                     saleorder_entry.sd.get(itemposistion).setOrgSalePrice(sale_price);
                     itemAdapter.CalculateCurrencyPrice(saleorder_entry.sd.get(itemposistion).getCode(),itemposistion);
                     sale_price=saleorder_entry.sd.get(itemposistion).getSale_price();
                 }
                 //added by KLM for currency on priceLevel 28062022
                 double amt= Double.parseDouble(txtqty.getText().toString())*sale_price;

                 SummaryFormat(txtnet,amt);
                 SummaryFormat(txtprice,saleorder_entry.sd.get(itemposistion).getSale_price());
                ...
                {
                    returnin_entry.sd.get(itemposistion).setPriceLevel(priceLevels.get(position).getLevel_name());
                    returnin_entry.txtShowSP.setText(priceLevels.get(position).getLevel_name());
                    double sale_price=getSalePrice(priceLevels.get(position).getLevel_name());
                    returnin_entry.sd.get(itemposistion).setSale_price(sale_price);
                    returnin_entry.sd.get(itemposistion).setDis_price(sale_price);
                    //added by KLM for currency on priceLevel 28062022
                    if(frmmain.use_multicurrency){
                        returnin_entry.sd.get(itemposistion).setOrgSalePrice(sale_price);
                        itemAdapter.CalculateCurrencyPrice(returnin_entry.sd.get(itemposistion).getCode(),itemposistion);
                        sale_price=returnin_entry.sd.get(itemposistion).getSale_price();
                    }
                    //added by KLM for currency on priceLevel 28062022
                    double amt= Double.parseDouble(txtqty.getText().toString())*sale_price;

                    SummaryFormat(txtnet,amt);
                    SummaryFormat(txtprice,returnin_entry.sd.get(itemposistion).getSale_price());
                    returnin_entry.getSummary();



                    row_index=position;
                    returnin_entry.pad.notifyDataSetChanged();
                }
             else
             {
                 ...
                 //added by KLM for currency on priceLevel 28062022
                 if(frmmain.use_multicurrency){
                     sale_entry.sd.get(itemposistion).setOrgSalePrice(sale_price);
                     itemAdapter.CalculateCurrencyPrice(sale_entry.sd.get(itemposistion).getCode(),itemposistion);
                     sale_price=sale_entry.sd.get(itemposistion).getSale_price();
                 }
                 //added by KLM for currency on priceLevel 28062022
                 double amt= Double.parseDouble(txtqty.getText().toString())*sale_price;
