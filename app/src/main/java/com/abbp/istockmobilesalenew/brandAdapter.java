    ...
    public void onBindViewHolder(brandAdapter.MyViewHolder holder, int position) {
        ...
        if(frmmain.withoutclass.equals("false")){
            ...
            else
            {
                ...
                {
                    holder.btn.setBackgroundResource(R.drawable.btngradient);
                    rv = returnin_entry.gridcodeview;
                }
                else {
                    holder.btn.setBackgroundResource(R.drawable.btngradient);
                    rv = returnin_entry.gridcodeview;
                ...
                try {
                    if (data.get(position).getBrandName() == "Back") {
                        if (frmmain.withoutclass.equals("false")) {
                            ...
                            {
                                rv = returnin_entry.gridclassview;
                                returnin_entry.usr_codes.clear();
                            }
                            else {
                        ...
                        {
                            if (returnin_entry.brands.size() > 0)
                                returnin_entry.brands.clear();
                        }
                        else {
                        ...
                        if (cursor != null && cursor.getCount() != 0) {
                            if (cursor.moveToFirst()) {
                                do {
                                    ...
                                    {
                                        returnin_entry.brands.add(new brand(brandIDid, brandNamename));
                                    }
                                    else {
                        ...
                        {
                            brandAdapter ad = new brandAdapter(context, returnin_entry.brands, rv);
                            rv.setAdapter(ad);
                        }
                        else {
...
/*
                        ...
                        if(formname=="SaleOrder")//added by YLT
                        {
                            ...
                            Cursor cursor = DatabaseHelper.rawQuery("select distinct usr_code,description,sale_price from Usr_Code where category2=" + data.get(position).getBrandName() + " AND unit_type = 1 order by code,usr_code");
                            ...
                            if (cursor != null && cursor.getCount() != 0) {
                                if (cursor.moveToFirst()) {
                                    do {
                                        ...
                                        saleorder_entry.usr_codes.add(new usr_code(usr_code, description,sale_price));
                                    } while (cursor.moveToNext());
                        ...
                        else if(formname=="ReturnIn")
                        {
                            ...
                            Cursor cursor = DatabaseHelper.rawQuery("select distinct usr_code,description,sale_price from Usr_Code where BrandID=" + data.get(position).getBrandID() + " AND unit_type = 1 order by usr_code");
                            if (sale_entry.usr_codes.size() > 0) returnin_entry.usr_codes.clear();
                            if (frmmain.withoutclass.equals("true")) {
                                returnin_entry.usr_codes.add(new usr_code("Back", "Back"));
                            }
                            if (cursor != null && cursor.getCount() != 0) {
                                if (cursor.moveToFirst()) {
                                    do {
                                        ...
                                        String description = cursor.getString(cursor.getColumnIndex("description"));
                                        Double sale_price = cursor.getDouble(cursor.getColumnIndex("sale_price"));
                                        returnin_entry.usr_codes.add(new usr_code(usr_code, description,sale_price));
                                    } while (cursor.moveToNext());
                            ...
                            if (cursor != null && cursor.getCount() != 0) {
                                if (cursor.moveToFirst()) {
                                    do {
                                        ...
                                        String description = cursor.getString(cursor.getColumnIndex("description"));
                                        Double sale_price = cursor.getDouble(cursor.getColumnIndex("sale_price"));
                                        sale_entry.usr_codes.add(new usr_code(usr_code, description,sale_price));
                                    } while (cursor.moveToNext());
