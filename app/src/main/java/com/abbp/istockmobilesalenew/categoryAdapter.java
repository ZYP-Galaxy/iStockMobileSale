...
//                        Modify by KLM to Show usrcode associated with user class 21032022
                        ...
                        if (formname == "SaleOrder")//added by YLT
                        {
                            ...
                            if (returnin_entry.class_items.size() > 0)
                                returnin_entry.class_items.clear();
                        } else {
...
//                                            if (returnin_entry.class_items.size() > 0)
                                        returnin_entry.class_items.add(new class_item(classid, name));
                                    } else {
                        ...
                        if (formname == "SaleOrder")//added by YLT
                        {
                            ...
                            classAdapter ad = new classAdapter(context, returnin_entry.class_items, rv, "ReturnIn");//added by YLT
                            rv.setAdapter(ad);
                        } else {
...
/*
                        ...
                        if (formname == "SaleOrder")//added by YLT
                        {
                            ...
                            Cursor cursor = DatabaseHelper.rawQuery("select distinct usr_code,description,sale_price from Usr_Code where category=" + data.get(position).getCategory() + " AND unit_type = 1 order by usr_code");
                            ...
                            cursor.close();
                            usrcodeAdapter ad = new usrcodeAdapter(context, returnin_entry.usr_codes, rv, data, "ReturnIn");
                            rv.setAdapter(ad);
                            GridLayoutManager gridLayoutManager = new GridLayoutManager(context.getApplicationContext(), 4);
                            rv.setLayoutManager(gridLayoutManager);
                        } else {
